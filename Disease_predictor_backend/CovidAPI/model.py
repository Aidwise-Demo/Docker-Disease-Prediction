...
def generate_heatmap(model, img_array):
    ...
    last_conv_layer = model.get_layer('block5_conv3')  # Replace with the actual layer name
...
async def predict_class_with_heatmap(path):
    ...
    new_model = tf.keras.models.load_model("CovidAPI/covidmodel")
    ...
    plt.savefig('CovidAPI/superimposed_image.png')
    # plt.show()
    ...
    if per == 1:
        return f'Covid Not Detected,{predict}'
    else:
        return f'Covid  Detected,{predict}'
